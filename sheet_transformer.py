...
class SheetTransformer:
    ...
    def lookup(self, spread_id, name=None, sheet_index=0):
        ...
        n_rows = (
            sheet.get("properties").get("gridProperties").get("rowCount")
            if name
            else HEADER_ROWS
        )
        ...
        result = (
            ...
            .get(
                ...
                range=f"{title}!A1:{self.column_to_letter(n_cols)}{n_rows}",
            )
            .execute()
        )
        ...
        if name:
            ...
            member_row, near_matches = self.get_nearest_matches(
        ...
        for i, project in enumerate(values[0]):
            ...
            if member_row:
                member_status = (
                    ": ❌"
                    if i >= len(member_row) or member_row[i] == ""
                    else f": {member_row[i].strip()}"
                )
                if member_status == ": x":
                    member_status = ": ✅"
    ...
    def add_assignment(self, spread_id, assignment, deadline, sheet_index=0):
        ...
        result = (
            self.service.values()
            .get(spreadsheetId=spread_id, range=f"{title}!A1:1")
            .execute()
        )
        ...
        result = (
            self.service.values()
            .get(spreadsheetId=spread_id, range=f"{title}!A:A")
            .execute()
        )
        ...
        self.service.values().clear(
            spreadsheetId=spread_id, range=f"{title}!{new_col}:{new_col}"
        ).execute()
        self.service.values().update(
            spreadsheetId=spread_id,
        ...
        self.service.batchUpdate(spreadsheetId=spread_id, body=body).execute()
        return
    ...
    def add_treasurer_assignment(self, assignment, sheet_index=5):
        ...
        result = (
            self.service.values()
            .get(spreadsheetId=TREASURER_SHEET, range=f"{title}!A1:1")
            .execute()
        )
        ...
        assignments = result.get("values")
        new_col_index = (
            len(assignments[0]) if assignments else TREASURER_PROJECT_COL_INDEX
        )
        new_col = self.column_to_letter(new_col_index + 1)

        result = (
            self.service.values()
            .get(spreadsheetId=TREASURER_SHEET, range=f"{title}!A:A")
            .execute()
        )
        ...
        self.service.values().clear(
            spreadsheetId=TREASURER_SHEET, range=f"{title}!{new_col}:{new_col}"
        ).execute()
        self.service.values().update(
            spreadsheetId=TREASURER_SHEET,
            valueInputOption="USER_ENTERED",
    ...
    def change_deadline(self, spread_id, assignment, deadline, sheet_index=0):
        ...
        result = (
            self.service.values()
            .get(spreadsheetId=spread_id, range=f"{title}!A1:{HEADER_ROWS}")
            .execute()
        )
    ...
    def get_lab_hours_by_name(self, spread_id, name, sheet_index=0):
        ...
        result = (
            self.service.values()
            .get(spreadsheetId=spread_id, range=f"{title}")
            .execute()
        )
        values = result.get("values")
            ...
            while row < len(values):
                ...
                for officer_name in officer_names:
                    ...
                    if officer_name.upper().startswith(name.upper().strip()):
                        ...
                        if officer_name not in officer_hours:
                            officer_hours[officer_name] = {}

                        # Track times by each day
                        if day not in officer_hours[officer_name]:
        ...
        for officer in officer_hours:
            ...
            for day, times in officer_hours[officer].items():
                hours_as_str.append(
                    f'{day}: {times[0] if len(times) == 1 else ", ".join(times)}'
                )
